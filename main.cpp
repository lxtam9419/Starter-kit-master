...
void generateRandomTrail(string **arr, int &row, int &column) // Assign random objects to the trails after alien's turn.
{
    for (int i = 0; i < row; ++i)
    {
        for (int j = 0; j < column; ++j)
...
string findClosestZombie(int &row, int &column, string **arr) // A calculater for finding the shortest distance between the alien and a zombie.
{
    int rowA, colA;
    int shortestDistance = INT_MAX; // Initialize shortestDistance to a large number.
    string shortestPoint;
    for (int i = 0; i < row; i++)
    {
        for (int j = 0; j < column; j++)
        {
    ...
    // cout << "The shortest Manhattan distance is between A and " << shortestPoint << " : " << shortestDistance << endl; // For testing purpose.
...
void alienAttack(Character::Zombie &zombies, Character::Alien &aliens, string &hitZombie, int **PositionZombie, string **arr)
{
    int hit = stoi(hitZombie);
    int attack = aliens.getAttack(0);
    if (attack >= zombies.getLife(hit - 1))
    {
        // cout << "test 1" << endl;
        zombies.eraseLife(hit - 1);
        zombies.insertLife(0, hit - 1);
        arr[PositionZombie[hit - 1][0]][PositionZombie[hit - 1][1]] = " ";
    }
    else
    {
        // cout << "test 2" << endl;
        int life = zombies.getLife(hit - 1) - attack;
        zombies.eraseLife(hit - 1);
        zombies.insertLife(life, hit - 1);
    }
    // cout << "test 3" << endl;
}

void hitObject(string &hit, Character::Alien &aliens, Character::Zombie &zombies, string &lastMove, int &row, int &column, string **arr, int &x, int &y, int **PositionZombie) // Display different outputs based on what it hits.
{
    string hitZombie;
    for (int i = 1; i <= zombies.sizeZombie(); i++)
    {
    ...
    else if (hit == "p")
    {
        ...
        int closestZombie = std::stoi(findClosestZombie(row, column, arr));
    ...
    {
        // cout << "Zombie" << hitZombie << endl;
        cout << endl;
        lastMove = hitZombie;
        alienAttack(zombies, aliens, hitZombie, PositionZombie, arr);
    }
}
...
int main()
{
    ...
    while (exit != true)
    {
        ...
        else if (command == "up" || command == "down" || command == "left" || command == "right")
        {
            ...
            hitObject(hit, aliens, zombies, lastMove, row, column, arr, x, y, PositionZombie);
            ...
            {
                if ((x == 0 && lastMove == "^") || (x == (row - 1) && lastMove == "v") || (y == 0 && lastMove == "<") || (y == (column - 1) && lastMove == ">") || (lastMove <= to_string(zombies.sizeZombie())))
                {
                    break;
                }
                else
            ...
            generateRandomTrail(arr, row, column);
            zombieMove(zombies, aliens, row, column, arr, PositionZombie, rowStart, zombie);
            // cout << zombies.accumulateZombieLife() << " " << aliens.getLife(0) << endl;      //for testing purpose
            // system("pause");
